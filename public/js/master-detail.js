...
const app = (function () {
  ...
  function renderData(school, system, department) {
    ...
    let dept_title = (department.is_extended_department==1)? `${department.title}（碩士班）<span class="badge badge-warning"> 重點產業系所 </span>`:`${department.title}（碩士班）`;
  ...
  function _init() {
    ...
    // 拿資料
    API.getDepartmentDetail(schoolId, 'master', id).then(response => {
      if (!response.ok) {
