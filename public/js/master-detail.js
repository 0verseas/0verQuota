...
const app = (function () {
  ...
  function renderData(school, system, department) {
    ...
    $deptTitle.html(`${department.title}（碩士班）`);
    $schoolEngTitle.html(_isProvide(school.eng_title) ? `${school.eng_title}` : `不提供 Doesn't Provide`);
    $deptEngTitle.html(_isProvide(department.eng_title) ? `${department.eng_title} (Master)` : `不提供 Doesn't Provide`);
  ...
  function _init() {
    ...
    // 拿資料
    API.getDepartmentDetail('master', schoolId, id).then(data => {
      // 放資料
