const app = (function () {
  ...
  function renderData(data){
    ...
    $schoolTitle.html(`${data.title} ${data.two_year_tech_departments[0].title}`);
    $schoolEngTitle.html(`${data.eng_title} ${data.two_year_tech_departments[0].eng_title}`);
    ...
    $deptId.html(data.two_year_tech_departments[0].id);
    $deptUrl.html(`<a href="${data.two_year_tech_departments[0].url}" target="_blank">${data.two_year_tech_departments[0].url}</a>`);
    $deptEngUrl.html(`<a href="${data.two_year_tech_departments[0].eng_url}" target="_blank">${data.two_year_tech_departments[0].eng_url}</a>`);
    $groupCode.html(data.two_year_tech_departments[0].group_code);
    $genderLimit.html(data.two_year_tech_departments[0].gender_limit);
    $mainGroup.html(`${data.two_year_tech_departments[0].main_group_data.title} ${data.two_year_tech_departments[0].main_group_data.eng_title}`);
    if (data.two_year_tech_departments[0].sub_group != null) {
      $subGroup.html(`${data.two_year_tech_departments[0].sub_group_data.title} ${data.two_year_tech_departments[0].sub_group_data.eng_title}`);
    } else {
      $subGroup.html(`無 None`);
    }
    $evalTitle.html(data.two_year_tech_departments[0].evaluation_level.title);
    $evalEngTitle.html(data.two_year_tech_departments[0].evaluation_level.eng_title);
    $deptHasSelfEnrollment.html(data.two_year_tech_departments[0].has_self_enrollment ? `<span class="oi oi-check"></span>` : `<span class="oi oi-x"></span>`);
    $deptHasSpecialClass.html(data.two_year_tech_departments[0].has_special_class ? `<span class="oi oi-check"></span>` : `<span class="oi oi-x"></span>`);
    $deptHasForeignSpecialClass.html(data.two_year_tech_departments[0].has_foreign_special_class ? `<span class="oi oi-check"></span>` : `<span class="oi oi-x"></span>`);
    $deptDescription.html(data.two_year_tech_departments[0].description);
    $deptEngDescription.html(data.two_year_tech_departments[0].eng_description);

    // render 審查項目
    if (data.two_year_tech_departments[0].application_docs.length == 0) {
      $('#nav-shenchaItem-tab').remove();
    }
    // 審查費用
    if (data.two_year_tech_departments[0].has_review_fee) {
      ...
      <div>
        ...
        <dl class="row">
          ...
          <dd class="col-md-8">
            <p>${data.two_year_tech_departments[0].review_fee_detail}</p>
            <p>${data.two_year_tech_departments[0].eng_review_fee_detail}</p>
          </dd>
        </dl>
      </div>
      <hr>
    ...
    for (let doc of data.two_year_tech_departments[0].application_docs) {
      let appendData = ``;
      let required = 'x';
      if (doc.required) {
        required = 'check';
  ...
  function getDepartmentDetail(id, schoolCode) {
    API.getDepartmentDetail('two-year', schoolCode, id).then(data => {
