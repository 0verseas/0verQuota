const app = (function () {
  ...
  function filterDepartmentList() {
    ...
    for (let school of filterSchools) {
      // 取得以符合過濾條件的系所列表
      school.two_year_tech_departments = school.two_year_tech_departments.filter(department => {
        // 學群條件（主要、次要學群其一）
  ...
  function _setDepartmentList(schoolData = []) {
    ...
    for (let school of schoolData) {
      for (let department of school.two_year_tech_departments) {
        ...
        // 設定簡便連結
        const schoolURL = `two-year-detail.html?id=${department.id}&school-id=${school.id}&tab=nav-schoolInfo`;
        const detailURL = `two-year-detail.html?id=${department.id}&school-id=${school.id}&tab=nav-deptInfo'`;
        const shenchaItemURL = `two-year-detail.html?id=${department.id}&school-id=${school.id}&tab=nav-shenchaItem`;
